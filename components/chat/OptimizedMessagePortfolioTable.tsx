"use client";

import React from 'react';
import { useDataFetch } from '@/hooks/useDataFetch';
import { MessagePortfolioTable } from './MessagePortfolioTable';

interface OptimizedMessagePortfolioTableProps {
  dataId: string;
}

export function OptimizedMessagePortfolioTable({ dataId }: OptimizedMessagePortfolioTableProps) {
  const { data, loading, error } = useDataFetch(dataId);

  if (loading) {
    return (
      <div className="mt-4">
        <div className="animate-pulse flex items-center gap-3 p-3 bg-blue-50 rounded-lg border border-blue-200">
          <div className="w-5 h-5 bg-blue-400 rounded-full"></div>
          <div className="flex-1">
            <div className="h-4 bg-blue-200 rounded w-32 mb-1"></div>
            <div className="h-3 bg-blue-100 rounded w-48"></div>
          </div>
        </div>
      </div>
    );
  }

  if (error) {
    return (
      <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg">
        <p className="text-red-600 text-sm">Failed to load portfolio data: {error}</p>
      </div>
    );
  }

  if (!data) {
    return null;
  }

  // Use the existing MessagePortfolioTable component with fetched data
  return (
    <MessagePortfolioTable 
      tokens={data.tokens} 
      nativeBalance={data.native_balance} 
    />
  );
}
