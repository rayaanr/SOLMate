...
export async function POST(req: Request) {
  try {
    ...
    debugLogger.log('chat_request', 'Received chat request', { prompt });
    ...
    if (
      ...
      try {
        ...
        debugLogger.log('response_ready', 'Enhanced response generated successfully');
        return result.toUIMessageStreamResponse();
      } catch (apiError) {
        debugLogger.logError('wallet_query_error', apiError, { prompt, intent });

        // Fallback response
        const result = await aiService.generateFallbackResponse(prompt);
        debugLogger.log('fallback_response', 'Using fallback response due to wallet error');
        return result.toUIMessageStreamResponse();
      }
    }
    ...
    // Handle general queries
      ...
      debugLogger.log('route_decision', 'Processing general query (no intent or unrecognized)');
